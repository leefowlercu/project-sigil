...
- Red: add or update a failing test or acceptance scenario that captures
  intended behavior.
...
- For submodule implementation verification, use the local workflow rules for
  the corresponding submodule (`sigil/` or `sigil-web/`) when present;
  otherwise follow that submodule's documented verification commands in
  `README.md`.
